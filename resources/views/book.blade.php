...
<head>
    ...
    <style>
        ...
        .details {
            margin: 20px auto;
            /* Center horizontally with auto margins and 20px top and bottom margin */
            padding: 10px;
            /* Add padding for spacing */
            width: 80%;
            /* Adjust the width as needed, e.g., 80% for responsiveness */
            max-width: 600px;
        ...
        .details p {
            ...
            /* Ensure long text wraps within the container */
            margin: 0;
        ...
        @media (max-width: 480px) {
            ...
            .calendar {
                width: 100%;
                /* Full width for the calendar */
                max-width: 320px;
                /* Set a maximum width to prevent it from being too wide */
                margin: 0 auto;
                /* Center align the calendar within the container */
                padding-right: 15px;
            ...
            .calendar-body {
                ...
                padding: 10px;
                /* Decrease padding for the calendar body to fit better on small screens */
            }
            ...
            .calendar-weekdays div {
                ...
                align-items: center;
                padding: 5px 0;
                /* Adjust padding inside each weekday cell */
            }

            .calendar-day {
                width: auto;
                /* Adjust width to auto for responsiveness */
                height: 40px;
                /* Adjust height to maintain a square shape */
                padding: 5px;
            ...
            .timepicker {
                width: 100%;
                /* Full width for timepicker */
                max-width: 320px;
                /* Set a maximum width to prevent it from being too wide */
                margin: 0 auto;
                /* Center align the timepicker within the container */
                display: flex;
                /* Use flexbox layout */
                flex-wrap: wrap;
            ...
            .timepicker div {
                ...
                /* Width to fit three divs per row, considering the gap */
                text-align: center;
                /* Center content within each slot */
                padding: 10px;
                box-sizing: border-box;
                /* Include padding and borders in the element's width */
            }
            ...
            .form-container {
                width: 100%;
                /* Full width for form container */
                max-width: 320px;
                /* Set a maximum width to prevent it from being too wide */
                margin: 0 auto;
                margin-left: 1px;
                /* Center align the form within the container */
            }
            ...
            .strong {
                min-height: 50px;
                /* Ensure a minimum height to prevent it from being cut off */
                margin-top: 40px;
                padding: 10px;
                width: 100%;
                box-sizing: border-box;
...
<body>
    ...
    <div class="details">
        ...
        <p class="name"> {{ $services->price }} @if (isset($services->max_price))
                - {{ $services->max_price }}
            @endif
            {{ $services->currency }}</p>
    ...
    <div class="container">
        <div class="calendar">
            ...
            <div class="strong">
                <p>Geselecteerde datum:<span id="displayedDate">Nog niet geselecteerd</span></p>
                <p>Geselecteerde tijd:<span id="displayedTime">Nog niet geselecteerd</span></p>
            </div>
        ...
        <div class="design-placeholder" id="design_placeholder" style="background-color:#fff;">
            <span style="color:#fff;"><strong style="color:#fff;"id="placeholder-date"></strong></span>
            <span style="color:#fff;">Niet beschikbaar</span>
        ...
        <div class="form-container">
            ...
            <form id="bookingForm" action="{{ route('booking.store') }}" method="POST">
                @csrf
                <input id="hidden-date" name="selected_date" type="hidden">
                <input id="hidden-time" name="selected_time" type="hidden">
                <input name="service_name" type="hidden" value="{{ $services->name }}">
                <input name="service_price" type="hidden" value="{{ $services->price }}">
                <input name="service_currency" type="hidden" value="{{ $services->currency }}">
                <div class="form-content">

                    <div class="container-19">
                        <div class="naam">Naam</div>
                        <div class="container-13">
                            <input class="rectangle-12" name="first_name" type="first_name" placeholder="Voornaam"
                                required>
                            <input class="rectangle-17" name="last_name" type="last_name" placeholder="Achternaam"
                                required>
                        </div>
                        <div class="emailadres">E-mailadres</div>
                        <input class="rectangle-13" name="email" type="email" placeholder="E-mailadres"
                            required>
                        ...
                        <div class="container-20">
                            ...
                            <input class="rectangle-14" id="phone" name="phone" type="tel"
                ...
                <button class="volgende" id="next-button" type="submit">Bevestiging</button>
    ...
    <script>
        ...
        currentDate.setHours(currentDate.getHours() + 3); // Add 3 hours to current time
        ...
        function updateCalendar() {
            ...
            for (let day = 1; day <= lastDayOfMonth.getDate(); day++) {
                ...
                const allDisabled = dayTimes.every(timeSlot => timeSlot.status !== 0);
                ...
                dayDiv.addEventListener('click', () => selectDate(dayDiv, allDisabled || date < today));
                calendarDays.appendChild(dayDiv);
            }

            // Automatically find and select the next available date
            ...
            while (!selectedDate) {
                ...
                // Stop if we reach the end of the month (optional safeguard)
                if (dateToCheck.getMonth() !== currentDate.getMonth()) {
                    break;
                }
            }

            // Automatically select the default date if determined
            if (selectedDate) {
                const defaultDayDiv = Array.from(calendarDays.children).find(div => div.getAttribute('data-date') ===
                    selectedDate);
                if (defaultDayDiv) {
                    selectDate(defaultDayDiv, false);
                }
            }
        }
        ...
        function populateTimeSlots(date) {
            ...
            let allDisabled = true; // Assume all slots are disabled until proven otherwise
            ...
            function renderTimeSlots() {
                ...
                dayTimes.slice(0, slotsToShow).forEach(({
                    time,
                    status
                }) => {
                    ...
                    // Calculate slot time as a Date object
                    const slotTime = new Date(`${date}T${formattedTime}:00`);

                    // Disable the slot if the status is 1 (taken), 2 (not available), or the slot time is in the past
                    if (status === 1 || status === 2 || slotTime < currentTime) {
                        slotDiv.classList.add('highlight', 'disabled');
                    } else {
                        allDisabled = false; // There's at least one available slot
                        slotDiv.addEventListener('click', () => {
                            document.querySelectorAll('.timepicker div.selected').forEach(el => el.classList
                                .remove('selected'));
        ...
        function selectDate(dayDiv, isDisabledDay) {
            ...
            const allDisabledOrPast = dayTimes.every(timeSlot => {
                const slotTime = new Date(`${date}T${timeSlot.time.slice(0, 5)}:00`);
                return slotTime <= currentTime || timeSlot.status !== 0;
            });

            // Automatically select the next available date if all times are disabled or in the past
            if (allDisabledOrPast) {
                dayDiv.classList.add('disabled');
                ...
                while (true) {
                    ...
                    const nextDayTimes = daysData[nextDateString] || [];

                    const allNextDayDisabledOrPast = nextDayTimes.every(timeSlot => {
                        const slotTime = new Date(`${nextDateString}T${timeSlot.time.slice(0, 5)}:00`);
                        return slotTime <= currentTime || timeSlot.status !== 0;
                    });

                    if (!allNextDayDisabledOrPast) {
                        const nextDayDiv = Array.from(calendarDays.children).find(div => div.getAttribute('data-date') ===
                            nextDateString);
                        if (nextDayDiv) {
                            selectDate(nextDayDiv, false);
                        }
                        return;
                    }
        ...
        // Call updateCalendar to initially load the calendar and potentially auto-select the first available date
        ...
        document.getElementById('next-button').addEventListener('click', function(event) {
            ...
            if (!selectedTime) {
                alert('Please select a time before submitting the form.');
                event.preventDefault(); // Prevent the form from submitting
            } else {
                // Disable the button to prevent multiple submissions
                this.disabled = true;

                this.style.backgroundColor = '#ccc'; // Example: Gray out the button
